...
from ncatbot.core.element import MessageChain, Image, Text
...
class SuperResolution(BasePlugin):
    ...
    async def request_super_resolution(self, image_url: str) -> str:
        ...
        """
        api_url = "https://siyangyuan-animecf.hf.space/api/predict"
    ...
    async def handle_super_resolution(self, group_id: int, image_url: str,messageid):
        """
        处理超分辨率逻辑。
        """
        ...
        if result_url:
            await self.api.post_group_msg(group_id, rtf=MessageChain([Image(result_url)]),reply=messageid)
        else:
    ...
    async def handle_group_message(self, event: GroupMessage):
        ...
        if raw_message.startswith("/超"):
            if "[CQ:image" in raw_message:  # 检查是否包含图片
                ...
                print("pending_super_resolution:", self.pending_super_resolution)
