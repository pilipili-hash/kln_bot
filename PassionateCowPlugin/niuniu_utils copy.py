...
async def execute_role_skill(db_path, qq_id, msg, bot):
    """执行角色技能"""
    ...
    if role == "伟哥执事":
        await wego_skill(db_path, qq_id, msg, bot)
    elif role == "牛牛神父":
        await cow_priest_skill(db_path, qq_id, msg, bot)
    elif role == "牛主教":
        await cow_bishop_skill(db_path, qq_id, msg, bot)
    elif role == "牛子子龙":
        await cow_zilong_skill(db_path, qq_id, msg, bot)
    elif role == "牛头神":
        await cow_head_skill(db_path, qq_id, msg, bot)
    elif role == "圣女":
        await saint_skill(db_path, qq_id, msg, bot)
    elif role == "修女":
        await nun_skill(db_path, qq_id, msg, bot)
    elif role == "色虐侍女":
        await sadistic_maid_skill(db_path, qq_id, msg, bot)
    elif role == "色虐领主":
        await sadistic_lord_skill(db_path, qq_id, msg, bot)
    elif role == "魅魔":
        await demon_skill(db_path, qq_id, msg, bot)
    elif role == "雅儿贝德":
        await yaerbode_skill(db_path, qq_id, msg, bot)
    elif role == "色虐":
        await sadistic_skill(db_path, qq_id, msg, bot)

async def wego_skill(db_path, qq_id, msg,bot):
    """伟哥执事技能：多重攻击"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
        opponents = await cursor.fetchall()
        if not opponents:
            await bot.api.post_group_msg(group_id=msg.group_id, text="没有其他玩家可以对战！")
            return
    ...
    await update_player_data(db_path, opponent_data["qq_id"], {
        "length": opponent_data["length"],
        "role": new_opponent_role,
        "item": opponent_data.get("item", "")
    })

    await  bot.api.post_group_msg(group_id=msg.group_id,text=f"伟哥执事发动技能：多重攻击！对 {opponent_data['qq_id']} 造成了 {total_damage} 点伤害，其牛子长度变为 {opponent_data['length']} cm，角色是 {new_opponent_role}")

async def cow_priest_skill(db_path, qq_id, msg,bot):
    """牛牛神父技能：鞭打"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"牛牛神父发动技能：鞭打！对 {opponent_data['qq_id']} 造成了 {damage} 点伤害，其牛子长度变为 {opponent_data['length']} cm，角色是 {new_opponent_role}")

async def cow_bishop_skill(db_path, qq_id, msg,bot):
    """牛主教技能：审判"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        ...
        f"你的牛子长度变为 {player_data['length']} cm，角色是 {new_player_role}。"
        f"对手 {opponent_data['qq_id']} 的牛子长度变为 {opponent_data['length']} cm，角色是 {new_opponent_role}"
    )

async def cow_zilong_skill(db_path, qq_id, msg,bot):
    """牛子子龙技能：牛胆"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    if player_abs > opponent_abs:
        ...
        await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"牛子子龙发动技能：牛胆！胜利！你的牛子长度变为 {temp_length} cm")
    else:
        # 失败：扣除对应取整数值
        ...
        await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"牛子子龙发动技能：牛胆！失败！你的牛子长度减少 {deduction} cm，变为 {player_data['length']} cm")
...
async def cow_head_skill(db_path, qq_id, msg,bot):
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"牛头神发动技能：神话之力！连续打胶10次成功！你的牛子长度变为 {player_data['length']} cm，角色是 {new_role}")

async def saint_skill(db_path, qq_id, msg,bot):
    """圣女技能：堕落"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    # 堕落逻辑：使对手的牛子长度变为负数
    opponent_data["length"] = -abs(opponent_data["length"])
    new_opponent_role = determine_role(opponent_data["length"])
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"圣女发动技能：堕落！使 {opponent_data['qq_id']} 的牛子长度变为 {opponent_data['length']} cm，角色是 {new_opponent_role}")

async def nun_skill(db_path, qq_id, msg,bot):
    """修女技能：守护"""
    player_data = await get_player_data(db_path, qq_id)
    # 守护逻辑：抵挡长度损失，并提高打胶成功率
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text="修女发动技能：守护！下次受到攻击时将抵挡长度损失，并且打胶成功率提高！")

async def sadistic_maid_skill(db_path, qq_id, msg,bot):
    """色虐侍女技能：腐蚀"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"色虐侍女发动技能：腐蚀！{opponent_data['qq_id']} 打胶时有80%概率缩短长度")

async def sadistic_lord_skill(db_path, qq_id, msg,bot):
    """色虐领主技能：混乱打击"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"色虐领主发动技能：混乱打击！对 {opponent_data['qq_id']} 造成了 {damage} 点伤害，其牛子长度变为 {opponent_data['length']} cm，角色是 {new_opponent_role}")

async def demon_skill(db_path, qq_id, msg,bot):
    """魅魔技能：引诱"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=
        f"魅魔发动技能：引诱！{opponent_data['qq_id']} 损失了 {cost} cm，你的牛子长度增加到 {player_data['length']} cm，角色是 {new_player_role}"
    )

async def yaerbode_skill(db_path, qq_id, msg,bot):
    """雅儿贝德技能：深渊"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"雅儿贝德发动技能：深渊！{opponent_data['qq_id']} 的牛子长度减少了 {reduction} cm，变为 {opponent_data['length']} cm，角色是 {new_opponent_role}")

async def sadistic_skill(db_path, qq_id, msg,bot):
    """色虐技能：吞噬"""
    player_data = await get_player_data(db_path, qq_id)
    async with aiosqlite.connect(db_path) as db:
        cursor = await db.execute("SELECT * FROM players WHERE qq_id != ?", (qq_id,))
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=
        f"色虐发动技能：吞噬！吸收了 {opponent_data['qq_id']} 的 {absorption} cm，你的牛子长度变为 {player_data['length']} cm，角色是 {new_player_role}"
    )
...
async def jj_battle(db_path, msg, bot):
    ...
    if player_abs > opponent_abs:
        result = "胜利"
        new_length = player_data["length"] + abs(opponent_data["length"]) * 0.3
    elif player_abs < opponent_abs:
        result = "失败"
        new_length = player_data["length"] - abs(opponent_data["length"]) * 0.3
    else:
        result = "平局"
        new_length = player_data["length"]
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=f"对战结果：{result}！你的牛子长度变为 {round(new_length, 3)} cm，角色是 {new_role}"
    )
...
async def query_player(db_path, msg, bot):
    ...
    await bot.api.post_group_msg(
        group_id=msg.group_id,
        text=(
            f"你的牛子长度是 {round(player_data['length'], 3)} cm，角色是 {player_data['role']}。\n"
